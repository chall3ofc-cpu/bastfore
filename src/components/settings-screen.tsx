...
export function SettingsScreen({ items, settings, onChange }: Props) {
  ...
  // FÖRENKLAD STATISTIK: Räknar bara antal och procent
  const stats = useMemo(() => {
    ...
    // Räknar ut hur många procent som äts upp
    ...
    setToast(
      matches.length === 0
        ...
        : {
            ...
            lines: matches.map(
              (m) =>
                `- ${m.name} (${daysLabel(m.expirationDate)} - ${actionHint(m.expirationDate)})`,
            ),
          },
    );
  ...
  return (
    <div className="relative px-5 pt-10">
      ...
      <p className="mt-1 text-sm text-muted-foreground">Statistik och larmkonfiguration</p>

      {/* RENSAD SEKTION: BARA PROCENT OCH ANTAL */}
        ...
        <div className="mt-4 h-3 w-full rounded-full bg-muted overflow-hidden">
          <div 
            className={cn(
              ...
              stats.rate > 80 ? "bg-fresh" : stats.rate > 50 ? "bg-warn" : "bg-danger"
            )}
      ...
      {toast && (
        <div className="fixed inset-x-4 top-4 z-50 mx-auto max-w-md animate-[toast-down_0.35s_cubic-bezier(0.22,1,0.36,1)] rounded-2xl border border-border bg-popover p-4 text-popover-foreground shadow-sheet">
          ...
          <button onClick={() => setToast(null)} className="mt-3 text-sm font-semibold text-primary">
            Stäng
          </button>
        </div>
      )}
